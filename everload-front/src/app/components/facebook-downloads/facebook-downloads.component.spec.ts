...
describe('FacebookDownloadsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      declarations: [FacebookDownloadsComponent, FakeTranslatePipe],
      providers: [
  ...
  describe('download() URL validation', () => {
    it('rejects URLs that are neither facebook.com nor fb.watch', () => {
      component.videoUrl = 'https://example.com/video';

      component.download();

      expect(component.error).toBe('INVALID_FACEBOOK_URL');
      expect(component.loading).toBeFalse();
      httpMock.expectNone(req => req.url.includes('/api/downloadFacebook'));
    });

    it('accepts facebook.com URLs and issues the download request', () => {
      component.videoUrl = 'https://www.facebook.com/reel/123';

      component.download();

      expect(component.error).toBeNull();
      expect(component.loading).toBeTrue();
      httpMock.expectOne(req => req.url.startsWith('/api/downloadFacebook')).flush(new Blob(['x']), {
  ...
  describe('saveToNas()', () => {
    ...
    it('queues the job, closes the picker and starts polling its status', fakeAsync(() => {
      ...
      expect(musicSpy.ytDlpQueueUrl).toHaveBeenCalledWith('https://www.facebook.com/reel/123', '', 1, 'Reels');
