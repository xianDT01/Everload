...
describe('TwitterDownloadsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      declarations: [TwitterDownloadsComponent, FakeTranslatePipe],
      providers: [
  ...
  describe('download() URL validation', () => {
    it('rejects URLs without a /status/ segment', () => {
      component.tweetUrl = 'https://twitter.com/user';

      component.download();

      expect(component.error).toBe('INVALID_TWITTER_URL');
      expect(component.loading).toBeFalse();
      httpMock.expectNone(req => req.url.includes('/api/downloadTwitter'));
    });

    it('accepts tweet URLs with a /status/ segment and issues the download request', () => {
      component.tweetUrl = 'https://twitter.com/user/status/12345';

      component.download();

      expect(component.error).toBeNull();
      expect(component.loading).toBeTrue();
      httpMock.expectOne(req => req.url.startsWith('/api/downloadTwitter')).flush(new Blob(['x']), {
  ...
  describe('saveToNas()', () => {
    ...
    it('queues the job, closes the picker and starts polling its status', fakeAsync(() => {
      ...
      expect(musicSpy.ytDlpQueueUrl).toHaveBeenCalledWith('https://twitter.com/user/status/12345', '', 1, 'Tweets');
